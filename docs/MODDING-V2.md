...
> **Critical constraint:** With kernel overlayfs via `unshare -rm`, the mount
> exists only inside that process's private mount namespace and dies when the
> process exits. The game process (launched separately by Steam/Proton) never
> sees it. This rules out `unshare -rm` as a standalone mount method.

| Priority | Method | How | Tradeoffs |
|---|---|---|---|
| **Primary** | Scoped root mount helper | `powos-mods-mount` binary + sudoers drop-in (exact-args allowlist, like `powos-container-ctl`) | Real system-wide mount. Survives Steam restarts. Works with pressure-vessel. Requires one-time sudo setup (`powos mods setup`). |
| **Fallback** | fuse-overlayfs | Persistent FUSE daemon in user namespace. Mount visible in normal namespace. | ~2x I/O overhead under Wine (measurable on large mod sets with many small files). No root needed. |

**`unshare -rm` is NOT used.** The namespace dies with the process; the game
never sees the mount.

**Sudoers mount helper (`powos-mods-mount`):**

```bash
# /usr/lib/powos/powos-mods-mount — installed by the image, not user-writable
# Exact-args sudoers pattern: only allows mount/umount of overlay at
# /tmp/powos-mods/*/merged with validated lowerdir paths.

# /etc/sudoers.d/powos-mods (installed by image)
powos ALL=(root) NOPASSWD: /usr/lib/powos/powos-mods-mount mount *
powos ALL=(root) NOPASSWD: /usr/lib/powos/powos-mods-mount umount *
```

The helper validates all arguments before calling `mount`:
- Mountpoint must match `/tmp/powos-mods/<game>/merged`
- All lowerdir paths must be under the Steam library or staging dir
- Upperdir/workdir must be under `/tmp/powos-mods/`
- Rejects any argument containing `;`, `&`, `|`, newlines

**fuse-overlayfs fallback** (when sudoers not configured):

```bash
fuse-overlayfs \
  -o "lowerdir=<mod-A>:<mod-B>:<game>,upperdir=<upper>,workdir=<work>" \
  /tmp/powos-mods/<game>/merged
# FUSE daemon stays running → mount persists → game sees it
```

This is the same approach used by
[linux-mod-organizer](https://codeberg.org/enoki/linux-mod-organizer), which
confirms fuse-overlayfs + Proton works on Steam Deck and desktop Linux.

**Detection at runtime:**
```bash
if [[ -x /usr/lib/powos/powos-mods-mount ]] && sudo -n /usr/lib/powos/powos-mods-mount check 2>/dev/null; then
    OVERLAY_METHOD="kernel"   # real mount via helper
elif command -v fuse-overlayfs &>/dev/null; then
    OVERLAY_METHOD="fuse"     # fuse-overlayfs daemon
else
    perr "No overlay method available. Run: powos mods setup"
    return 1
fi
```

### Steam integration — `powos-game-shim`

**One shim, not two wrappers.** The test harness (task-886cdab4) needs a
`%command%` wrapper for env-injection on DRM'd games launched via
`steam -applaunch`. The mod overlay also needs a `%command%` wrapper. Steam gets
ONE launch-options line — so both are the SAME shim: **`powos-game-shim`**.

```bash
# /usr/lib/powos/powos-game-shim — THE unified Steam launch wrapper
# Steam launch options: powos-game-shim %command%

#!/bin/bash
set -euo pipefail
APPID="${SteamAppId:-}"

# ── Phase 1: Ensure overlay mounted (mod system) ────────────────────
MERGED="/tmp/powos-mods/${APPID}/merged"
if [[ -f "$HOME/.local/state/powos/mods/manifests/${APPID}.json" ]]; then
    if ! mountpoint -q "$MERGED" 2>/dev/null; then
        powos mods _mount "$APPID"
    fi
    # Rewrite game dir to merged overlay
    # (game exe path in $@ points at original dir; replace prefix)
    GAME_DIR=$(powos mods _game_dir "$APPID")
    set -- "${@/$GAME_DIR/$MERGED}"
    cd "$MERGED"
fi

# ── Phase 2: Verify env sentinel (test harness, if active) ─────────
VERIFY_ENV="/tmp/powos-verify/${APPID}.env"
...
**Ownership:** The mod system owns `powos-game-shim`. The test harness
(task-886cdab4) writes its env sentinel to `/tmp/powos-verify/<appid>.env` and
the shim sources it — the harness never needs its own wrapper.
...
| File | Purpose |
|---|---|
| `lib/mods/core.sh` | Manifest CRUD, staging helpers |
| `lib/mods/nexus-api.sh` | Extracted Nexus API helpers (from install.sh dedup) |
| `lib/mods/deploy.sh` | Overlayfs mount/unmount, method detection, Steam launch options |
...
| `bin/powos-game-shim` | Unified Steam `%command%` wrapper (overlay + verify env) |
| `bin/powos-mods-mount` | Scoped root mount helper (exact-args sudoers pattern) |
| `config/mods/games.d/*.conf` | Per-game install rules (start with Cyberpunk, Skyrim SE, GTA V) |
| `config/sudoers.d/powos-mods` | Sudoers drop-in for powos-mods-mount |
...
2. **Flatpak Steam:** RESOLVED — Flatpak Steam cannot see mounts outside its
   sandbox without `--filesystem` overrides, which are fragile across updates.
   v1 detects Flatpak Steam and refuses `deploy` with a clear message:
   `"Steam is installed as a Flatpak. Overlay deploy requires native Steam
   (rpm/system package). On Bazzite, Steam is the system package by default."`
   Bazzite ships native Steam, so this is a non-issue on PowOS's target platform.
...
- **[Fluorine-Manager](https://github.com/SulfurNitride/Fluorine-Manager):**
  ...
  `pressure-vessel` namespace conflict: Steam's Proton runtime
  (`pressure-vessel-wrap`) creates its own mount namespace, so an overlay
  mounted in a separate user namespace (`unshare -rm`) is invisible to the
  game process. Workaround on NixOS: `nsenter` the launcher's namespace via
  `lsns -o pid,type,user,command`. Our solution avoids the problem entirely:
  the sudoers mount helper creates a real system-wide mount (visible in all
  namespaces), and fuse-overlayfs creates a persistent FUSE mount (also
  visible system-wide). Neither requires namespace tricks.
