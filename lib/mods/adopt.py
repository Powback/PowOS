#!/usr/bin/env python3
# adopt.py — the scanner behind `powos mods adopt`.
#
# Lifted verbatim out of a `python3 - <<'PY'` heredoc inside
# mods_adopt_cmd(). It was 297 of that function's 335 lines, and
# build/complexity.py — which does not know about heredocs — was counting
# every `for`/`if`/`elif` in here as shell branching: the function measured
# CC 53 while the surrounding shell is CC 8. A file also means the scanner
# is lintable, importable and runnable on its own.
#
# Ships beside adopt.sh in both the image (COPY lib/ -> /usr/lib/powos/) and
# a source checkout, the same way vu-rcon.py / vu-cdp.py do.
#
# Usage: adopt.py <game_dir> <game> <staging_base> <manifest_path>
#                 <dry_run:True|False> <appid> [nexus_slug]
